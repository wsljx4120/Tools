...
(function (window) {
  ...
  for (var l = 0; l < 26; l++) {
      ...
      arr_ImageLetters[l].src = "/images/free-alphabet-clipart-" + arr_letters[l] + ".jpg"
  }
          ...
          for (j = 0; j < Math.floor(imageData.height); j = j + sqWidth) {
              for (i = 0; i < Math.floor(imageData.width); i = i + sqWidth) {
                  ...
                  if (nAvg > 60 && nIndex < carray.length) {
                      carray[nIndex].destX = (j + Math.floor(sqWidth / 2)) * 1.5+100;
                      carray[nIndex].destY = (i + Math.floor(sqWidth / 2)) * 1.5;
                      nIndex++
                  }
              }
          }
  ...
  function draw() {
      ...
      for (i = 0; i < carray.length; i++) {
          ...
          if (C.destX > -1) {
              ...
              C.y += (C.destY - C.y) / 4 + ((C.destY - C.y) / 90 * C.vy) + nGeneralWindY+10;
              C.alpha += (.1 - C.alpha) / 2;
          } else {
          ...
          g.beginPath();
          g.fillStyle = "#FFFFFF";
          g.arc(C.x, C.y*0.8, C.r, 0, Math.PI*2 , true);
          g.closePath();
          g.fill();
  ...
  function setHighNumCircles() {
      NUM_CIRCLES = 1500;
      bHighRes = true;
      delete carray; ($("#radio_Low")).attr("checked", "false"); ($("#radio_High")).attr("checked", "true");
      carray = new Array();
      RADIUS = 3;
      for (i = 0; i < NUM_CIRCLES; i++) carray[i] = new Circle(Math.floor(Math.random() * WIDTH), Math.floor(Math.random() * HEIGHT), RADIUS)
  }
  function setMinNumCircles() {
      NUM_CIRCLES = 300;
      bHighRes = false;
      RADIUS = 8; ($("#radio_Low")).attr("checked", "true"); ($("#radio_High")).attr("checked", "false");
      delete carray;
      carray = new Array();
      for (i = 0; i < NUM_CIRCLES; i++) carray[i] = new Circle(Math.floor(Math.random() * WIDTH), Math.floor(Math.random() * HEIGHT), RADIUS)
  }
